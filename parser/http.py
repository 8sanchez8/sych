...
# -*- coding: utf-8 -*-
# Файл изменён 2016-02-26: Убрана проверка длины, чтобы не падало на попытках медленного DDoS
...
def parse_headers(f):
    ...
    while 1:
        line = f.readline()
        if not line:
            raise dpkt.dpkt.NeedData('premature end of headers')
        line = line.strip()
        if not line:
            break
        l = line.split(':', 1)
        if len(l[0].split()) != 1:
            raise dpkt.dpkt.UnpackError('invalid header: %r' % line)
        k = l[0].lower()
    ...
    if headers.get('transfer-encoding', '').lower() == 'chunked':
        ...
        while 1:
            try:
                ...
                raise dpkt.dpkt.UnpackError('missing chunk size')
            n = int(sz, 16)
            if n == 0:
                found_end = True
            buf = f.read(n)
            ...
            if n and len(buf) == n:
                ...
                break
        if not found_end:
            raise dpkt.dpkt.NeedData('premature end of chunked body')
        body = ''.join(l)
...
class Message(dpkt.dpkt.Packet):
...
class Request(Message):
    ...
    def unpack(self, buf):
        ...
        l = line.strip().split()
        if len(l) < 2:
            raise dpkt.dpkt.UnpackError('invalid request: %r' % line)
        if l[0] not in self.methods:
            raise dpkt.dpkt.UnpackError('invalid http method: %r' % l[0])
        if len(l) == 2:
            # HTTP/0.9 does not specify a version in the request line
            self.version = '0.9'
        else:
            if not l[2].startswith(self.__proto):
                raise dpkt.dpkt.UnpackError('invalid http version: %r' % l[2])
...
class Response(Message):
    ...
    def unpack(self, buf):
        ...
        if len(l) < 2 or not l[0].startswith(self.proto) or not l[1].isdigit():
            raise dpkt.dpkt.UnpackError('invalid response: %r' % line)
        self.version = l[0][len(self.proto) + 1:]
        self.status = l[1]
        self.reason = l[2] if len(l) > 2 else ''
        Message.unpack(self, f.read())

    def __str__(self):
        return '%s/%s %s %s\r\n' % (self.proto, self.version, self.status,
